...
class BGGItemDetails:
    ...
    def __clean_details_response(response) -> dict:
        ...
        thumbnail_url = thumbnail_url_item.text.strip() if thumbnail_url_item is not None else None
        image_url_item = item.find('./image')
        image_url = image_url_item.text.strip() if image_url_item is not None else None
